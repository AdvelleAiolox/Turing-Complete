...
第一个位输入用于控制模块的开启与关闭，第二三输入为字节输入，为输入1和输入2，用于比较，若输入1小于输入2，则第一个输出为真，若输入1等于输入2，则第二个输出为真，若输入1大于输入2，则第三个输出为真
...
在RAM存32个数，然后再从头至尾输出这32个数

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/RAM.JPG)

**Delay**

延迟

门分数为门数量，延迟分数为通过最多的门的数量乘2

此关需要5门和6延迟（最多通过3门）

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Delay.JPG)

**The Product of Nibbles**

乘法

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/The%20Product%20of%20Nibbles.JPG)
...
![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Stack.JPG)

**Divide**

除法

输入两个字节，第一个是被除数，第二个是除数，输出商和余数

首先偷懒，尝试用减法编程暴力做

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide.JPG)

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide%201.JPG)

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide%202.JPG)

每四个语句为一块，第一块将输入1存入寄存器0，第二块将输入2存入寄存器1，判断寄存器0是否小于寄存器1，否则减去寄存器1的值并且给寄存器2加一

最后输出寄存器2的数值，为商，寄存器0的数值，为余数

但是毕竟是编程，比较慢

电路法

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide%203.JPG)

首先设计一个移位模块，但比普通移位模块多一个是否溢出的判断

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide%205.JPG)

![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Divide%204.JPG)

计算部分细节，从最高位起，如果能减除数则减，并在商里记录，然后减完的值再进行同样操作，如果不能减则直接进行下一位操作
...
![image](https://github.com/AdvelleAiolox/Turing-Complete/blob/main/Photos/Functions/Push%20and%20Pop.JPG)
